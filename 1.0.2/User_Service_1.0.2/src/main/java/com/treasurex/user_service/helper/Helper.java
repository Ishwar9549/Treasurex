package com.treasurex.user_service.helper;
...
import com.treasurex.user_service.client.AppConfigFeignClient;
import com.treasurex.user_service.service.EmailService;
...
public class Helper {
	...
	private final AppConfigFeignClient appConfigFeignClient;
	...
	public String generateOtp() {
		...
		String otpMinLen;
		try {
			otpMinLen = appConfigFeignClient.getAppConfigByName("OTP_MIN_LENGTH").getData().getValue();
		} catch (Exception e) {
			otpMinLen = "4";
		}
	...
	public int minPasswordLen() {
		...
		try {
			passwordMinLen = appConfigFeignClient.getAppConfigByName("PASSWORD_MIN_LENGTH").getData().getValue();
	...
	public int minUserIDLen() {
		String passwordMinLen;
		try {
			passwordMinLen = appConfigFeignClient.getAppConfigByName("USERID_MIN_LENGTH").getData().getValue();
			return Integer.valueOf(passwordMinLen);
		} catch (Exception e) {
			passwordMinLen = "6";
			return Integer.valueOf(passwordMinLen);
