...
public class AdminService {
	 ...
	 */
	public String approve(ApprovalRequest approvelRequest) {
		User user = findUserByLoginId(approvelRequest.getLoginId());
		...
		if (user.getTypeOfUser().equals("NORMAL_USER")) {
			throw new RuntimeException(
					"User " + approvelRequest.getLoginId() + " is Normal User Approvel / Reject is not allowed");
		}
