...
public class UserMapper {
	...
	public User dtoToEntity(RegisterRequest request) {
		...
		User user = User.builder().userId(request.getUserId()).firstName(request.getFirstName())
		...
		if (request.getSecurityQuestions() != null) {
			List<SecurityQuestion> questions = request.getSecurityQuestions().stream().map(q -> SecurityQuestion
					.builder().question(q.getQuestion()).answerHash(hashAnswer(q.getAnswer())).user(user).build())
					.toList();
