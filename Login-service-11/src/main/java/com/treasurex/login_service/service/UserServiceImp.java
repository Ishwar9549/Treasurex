...
public class UserServiceImp implements UserService {
	...
	public String register(RegisterRequest registerRequest) {
		...
		log.info("Attempting to register user with User Id: {}", registerRequest.getUserId());
	...
	@Override
	public String verifyUserByOtp(VerifyRequest verifyRequest) {

		log.info("Attempting to verify user with user Id: {}", verifyRequest.getUserId());

		User user = userRepository.findByUserId(verifyRequest.getUserId()).orElseThrow(
				() -> new ResourceNotFoundException("User not found with user Id: " + verifyRequest.getUserId()));
		...
		log.info("User verified successfully for user Id: {}", user.getUserId());

		return "User " + user.getUserId() + " has been verified successfully. You can now log in.";
	}

	@Override
	public String login(LoginRequest loginRequest) {

		log.info("Attempting login for user Id: {}", loginRequest.getUserId());
	...
	@Override
	public String forgotPasswordByOtp(ForgotPasswordByOtpRequest forgotPasswordByOtpRequest) {
		log.info("Forgot password request received for user Id: {}", forgotPasswordByOtpRequest.getUserId());

		User user = userRepository.findByUserId(forgotPasswordByOtpRequest.getUserId())
				.orElseThrow(() -> new ResourceNotFoundException(
						"User not found with user Id: " + forgotPasswordByOtpRequest.getUserId()));
		...
		log.debug("Generated password reset OTP for {}: {}", user.getEmail(), otp);

		return "OTP has been sent to your email address (valid for 5 minutes).";
	}

	@Override
	public String resetPasswordByOtp(ResetPasswordByOtpRequest resetPasswordByOtpRequest) {
		log.info("Reset password request received for user Id: {}", resetPasswordByOtpRequest.getUserId());

		User user = userRepository.findByUserId(resetPasswordByOtpRequest.getUserId())
				.orElseThrow(() -> new ResourceNotFoundException(
						"User not found with user Id: " + resetPasswordByOtpRequest.getUserId()));
		...
		log.debug("Password updated successfully for user Id: {}", resetPasswordByOtpRequest.getUserId());
	...
	public String changePassword(ChangePasswordRequest changePasswordRequest) {
		log.info("Change password request received for user Id: {}", changePasswordRequest.getUserId());
		...
		log.debug("Password changed successfully for user Id: {}", changePasswordRequest.getUserId());
	...
	public String otpResend(ReSendOtpRequest reSendOtpRequest) {
		log.info("OTP resend request received for user Id: {}", reSendOtpRequest.getUserId());
		...
		log.debug("OTP resent to email: {}", user.getEmail());
