...
public class UserServiceImp implements UserService {
	...
	public ApiResponse<Map<String, String>> verifyPhoneNumber(String authorizationHeader, OtpVerifyRequest request) {
		...
		case FORGOT_MPIN:
			...
			break;
		default:
			log.error("Unexpected OTP purpose {} for phone={}", user.getOtpPurpose(), user.getPhoneNumber());
			throw new InvalidOtpPurposeException("Unexpected OTP purpose.");
		}
	...
	public ApiResponse<Map<String, String>> createPassword(String authorizationHeader, PasswordRequest request) {
		...
		if (!request.getNewPassword().equals(request.getConfirmPassword())) {
			log.warn("New password and confirm password mismatch for phone={}", user.getPhoneNumber());
	...
	public ApiResponse<Map<String, String>> setUserDetails(String authorizationHeader, UserDetailsRequest request) {
		...
		// Ensure email and phone is verified
		requirePhoneVerified(user);
	...
	public ApiResponse<Map<String, String>> login(LoginRequest loginRequest) {
		...
		if (user == null) {
			log.warn("Login failed: user not found for identifier={}", loginRequest.getLoginId());
			throw new NotFoundException("User not found with identifier: " + loginRequest.getLoginId());
		}
	...
	/**
	 * Extracts the JWT purpose from the Authorization header.
	 * 
	 * @param authorizationHeader The HTTP Authorization header containing the
	 *                            Bearer token.
	 * @return JwtPurpose enum indicating the intended purpose of the token.
	 * @throws InvalidCredentialsException if the header is missing, malformed,
	 *                                     expired, or contains an invalid purpose.
	 */
	private JwtPurpose extractPurpose(String authorizationHeader) {

		log.debug("Extracting JWT purpose from authorization header");
		...
		try {
			JwtPurpose purpose = JwtPurpose.valueOf(purposeClaim);
			log.debug("JWT purpose extracted successfully: {}", purpose);
			return purpose;
		} catch (IllegalArgumentException ex) {
	...
	/**
	 * Extracts the User entity from a JWT token in the Authorization header.
	 * Validates the token, ensures it has the expected purpose, and fetches the
	 * user by phone number.
	 *
	 * @param authorizationHeader The HTTP Authorization header containing the
	 *                            Bearer token.
	 * @param expectedPurpose     The expected JwtPurpose for this operation.
	 * @return User corresponding to the token's subject.
	 * @throws InvalidCredentialsException if the token is missing, invalid,
	 *                                     expired, or has the wrong purpose.
	 * @throws ResourceNotFoundException   if no user is found for the token's
	 *                                     subject.
	 */
	private User getUserFromTokenHeader(String authorizationHeader, JwtPurpose expectedPurpose) {

		log.debug("Extracting user from authorization header");
		...
		if (actualPurpose != expectedPurpose) {
			log.warn("JWT purpose mismatch. Expected: {}, Actual: {}", expectedPurpose, actualPurpose);
			throw new AccessDeniedException("Invalid token purpose");
		}
